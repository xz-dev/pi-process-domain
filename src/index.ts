...
type EmbeddedRootHost = typeof globalThis & {
  [PROCESS_DOMAIN_OWNER]?: EmbeddedRoot;
};
...
export async function openDomain(options?: Partial<OpenDomainOptions>): Promise<OpenDomainResult> {
  ...
  try {
    const host = globalThis as EmbeddedRootHost;
    ...
    if (owned !== undefined && existing !== null && !sameDeclaration(existing, owned.declaration)) {
      throw new ProcessDomainFatalError(
        "INVALID_DECLARATION",
        "process already owns a different embedded process domain",
      );
    }
    ...
    if (root !== undefined) {
      ...
      try {
        ...
        root.references -= 1;
        if (root.references === 0) await closeEmbeddedRoot(host, root);
        throw error;
      }
    }
    ...
    if (client === null) {
      ...
      else try { onFatal(fatal); } catch { /* preserve the original typed failure */ }
    }
    throw fatal;
  }
}

function ownedDomain(client: DomainClient, host: EmbeddedRootHost, root: EmbeddedRoot): ProcessDomain {
  let closed = false;
  return {
...
async function closeEmbeddedRoot(host: EmbeddedRootHost, root: EmbeddedRoot): Promise<void> {
  if (root.closing !== null) return root.closing;
  root.closing = (async () => {
    await root.broker.close();
    if (
      process.env[ENV_NAMES.DOMAIN_ID] === root.declaration.domainId &&
      process.env[ENV_NAMES.PROTOCOL] === `${root.declaration.protocolMajor}.${root.declaration.protocolMinor}`
    ) clearDeclaration();
    if (host[PROCESS_DOMAIN_OWNER] === root) delete host[PROCESS_DOMAIN_OWNER];
  })();
  return root.closing;
}

function sameDeclaration(left: DomainDeclaration, right: DomainDeclaration): boolean {
