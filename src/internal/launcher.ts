...
export function lockDir(): string {
  ...
  if (endpoint.platform === "unix") return path.join(endpoint.runtimeDir, "election");
...
export async function waitForBroker(timeoutMs: number): Promise<void> {
  ...
  for (;;) {
    ...
    if (await canConnect(ep.endpointPath)) return;
    await new Promise((r) => setTimeout(r, 100));
  }
}

function canConnect(endpointPath: string): Promise<boolean> {
