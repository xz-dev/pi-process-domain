...
export function protocolString(): string {
  return `${PROCESS_DOMAIN_PROTOCOL_MAJOR}.${PROCESS_DOMAIN_PROTOCOL_MINOR}`;
}

const ID_REGEX = /^[A-Za-z0-9_-]+$/;

function parseDeclaration(): DomainDeclaration | null {
  ...
  if (id === undefined && key === undefined && proto === undefined) return null;
  ...
  if (id === undefined || key === undefined || proto === undefined) {
    throw new ProcessDomainFatalError(
      "INVALID_DECLARATION",
      "domain declaration is incomplete: all of ID, KEY and PROTOCOL must be set together",
    );
  }
  ...
  if (!keyBytes) {
    throw new ProcessDomainFatalError(
      "INVALID_DECLARATION",
      "domain key must decode to exactly 32 bytes",
    );
  }
  ...
  if (major !== PROCESS_DOMAIN_PROTOCOL_MAJOR) {
    throw new ProcessDomainFatalError(
      "PROTOCOL_MISMATCH",
      `protocol major ${major} is incompatible with supported ${PROCESS_DOMAIN_PROTOCOL_MAJOR}`,
    );
  }
...
export function encodeReservationClaim(token: Uint8Array): string {
  return base64url(token);
}

export function decodeReservationClaim(value: string | undefined): Uint8Array | null {
