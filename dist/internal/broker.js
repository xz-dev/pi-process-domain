...
export class Broker {
    ...
    constructor(options) {
        ...
        if (options.domain) {
            this.domains.set(options.domain.domainId, newDomain(options.domain.domainId, deriveDomainAuthKey(options.domain.domainKey, options.domain.domainId)));
        }
        ...
        try {
            ...
            if (code === "EADDRINUSE") {
                ...
                if (this.options.domain !== undefined) {
                    throw new ProcessDomainFatalError("BROKER_UNAVAILABLE", `embedded broker endpoint is already in use: ${ep}`, err);
                }
                ...
                throw new ProcessDomainFatalError("BROKER_UNAVAILABLE", `broker endpoint already in use by a live broker: ${ep}`, err);
            }
            throw err;
        }
    }
    ...
    async onHandshake(conn, frame) {
        ...
        if (frame.t === "prove") {
            ...
            if (clientNonceRaw === null || brokerNonceRaw === null || theirMacRaw === null ||
            ...
            if (conn.handshakeHandler)
                raw.removeHandler(conn.handshakeHandler);
        ...
        const local = {
            join: async (args) => {
                ...
                if (!lease) {
                    ...
                    if (!resumeBytes)
                        throw new ProcessDomainFatalError("LEASE_REJECTED", "failed to issue resume capability");
                    lease = {
                        participantId,
                        incarnation: incarnationRaw,
                        resumeKeyHash: hashToken(resumeBytes),
            ...
            reserveSpawn: async (args) => {
                ...
                for (const r of state.reservations.values()) {
                    if (r.creatorParticipantId === p.participantId)
                        count += 1;
                }
     ...
     */
    startHeartbeat(conn, state) {
        if (conn.heartbeatTimer)
            clearInterval(conn.heartbeatTimer);
        if (conn.sweepTimer)
            clearInterval(conn.sweepTimer);
        const sweep = () => {
            const p = conn.participantId ? state.participants.get(conn.participantId) : undefined;
            if (!p)
                return;
        ...
        conn.sweepTimer = setInterval(() => {
            if (this.recomputeCertain(state))
                this.broadcastSnapshot(state);
        }, 1000);
    }
    ...
    recomputeCertain(state) {
        ...
        if (!uncertain) {
            for (const p of state.participants.values()) {
                ...
                if (Date.now() - p.lastSeen > SUSPECT_MS) {
                    uncertain = true;
                    break;
                }
            }
    ...
    removeParticipant(conn, state) {
        if (conn.participantId) {
            const p = state.participants.get(conn.participantId);
            if (p?.incarnation === conn.incarnation)
                this.expireParticipant(state, conn.participantId);
        }
        ...
        if (conn.participantId && conn.domainState) {
            ...
            if (p && p.incarnation === conn.incarnation) {
                ...
                const expiryTimer = setTimeout(() => {
                    ...
                    if (nowP && !nowP.connected)
                        this.expireParticipant(state, pid);
                }, EXPIRE_MS);
    ...
    close() {
        ...
        for (const state of this.domains.values()) {
            for (const reservation of state.reservations.values())
                this.clearReservationTimer(reservation);
