...
export function canonicalize(value, depth = 0) {
    if (depth > MAX_DEPTH)
        throw new Error(`nesting deeper than ${MAX_DEPTH}`);
    if (value === null)
        return null;
    ...
    if (typeof value === "number") {
        if (!Number.isFinite(value))
            throw new Error("non-finite number cannot be encoded");
    ...
    if (Array.isArray(value)) {
        if (value.length > MAX_CONTAINER_KEYS)
            throw new Error("array too large to encode");
    ...
    if (typeof value === "object") {
        ...
        if (keys.length > MAX_CONTAINER_KEYS)
            throw new Error("object too large to encode");
        const out = {};
    ...
    while (s.pos < s.input.length) {
        ...
        if (ch === "\\") {
            ...
            if (s.pos >= s.input.length)
                throw new Error("truncated escape");
            ...
            switch (esc) {
                ...
                case "t":
                    ...
                    break;
                case "u": {
                    if (s.pos + 4 >= s.input.length)
                        throw new Error("truncated unicode escape");
...
function parseNumber(s) {
    ...
    if (!Number.isSafeInteger(value))
        throw new Error("number out of safe integer range");
    if (Object.is(value, -0))
        throw new Error("negative zero not allowed");
    return value;
}
function parseValue(s, depth) {
    if (depth > MAX_DEPTH)
    ...
    if (ch === "t") {
        if (s.input.startsWith("true", s.pos)) {
            s.pos += 4;
            return true;
        }
    ...
    if (ch === "f") {
        if (s.input.startsWith("false", s.pos)) {
            s.pos += 5;
            return false;
        }
        throw new Error("invalid literal");
    ...
    if (ch === "n") {
        if (s.input.startsWith("null", s.pos)) {
            s.pos += 4;
            return null;
        }
        throw new Error("invalid literal");
...
function parseArray(s, depth) {
    ...
    for (;;) {
        if (out.length >= MAX_CONTAINER_KEYS)
 ...
 */
export function parseCanonicalObject(input) {
    const s = { input, pos: 0 };
    const value = parseValue(s, 0);
    skipWs(s);
    if (s.pos !== input.length)
        throw new Error("trailing characters after payload");
    if (!isPlainObject(value))
        throw new Error("frame payload must be a single JSON object");
    const obj = value;
...
export function isDecimalIntString(value) {
    if (typeof value !== "string")
        return false;
